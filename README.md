<div align="center">

# 👋 Hi, wir sind **Emin**! 

### *Cybersecurity Experten & Software Entwickler aus Berlin*

[![Kotlin](https://img.shields.io/badge/Kotlin-0095D5?logo=kotlin&logoColor=white)](https://kotlinlang.org/)
[![React](https://img.shields.io/badge/React-20232A?logo=react&logoColor=61DAFB)](https://react.dev/)
[![Node.js](https://img.shields.io/badge/Node.js-43853D?logo=node.js&logoColor=white)](https://nodejs.org/)
[![ESP32](https://img.shields.io/badge/ESP32-E7352C?logo=espressif&logoColor=white)](https://www.espressif.com/)
[![Security+](https://img.shields.io/badge/CompTIA-Security+-red)](https://www.comptia.org/)

</div>

---

## 🚀 Wer wir sind

Wir sind **Abdi & Emin** - ein dynamisches Duo aus Berlin mit Leidenschaft für:
- 🔒 **Cybersecurity** & Penetration Testing
- 💻 **Software Development** (Full-Stack)
- 🔌 **IoT & Embedded Systems** (ESP32, Arduino)
- 🛡️ **Security Operations** (SOC)

**Standort:** Berlin, Deutschland 📍  
**Kontakt:** info@emin-security.de 📧

---

## 🛠️ Tech Stack & Skills

### 💻 Programmiersprachen
| Sprache | Level | Projekte |
|---------|-------|----------|
| **Kotlin** | ⭐⭐⭐⭐⭐ | Android Apps, Backend |
| **Java** | ⭐⭐⭐⭐⭐ | Enterprise, Spring Boot |
| **JavaScript/React** | ⭐⭐⭐⭐⭐ | Web Apps, Dashboards |
| **Swift** | ⭐⭐⭐⭐☆ | iOS Entwicklung |
| **C#** | ⭐⭐⭐⭐☆ | .NET Anwendungen |
| **Python** | ⭐⭐⭐⭐☆ | Security Tools, Scripts |
| **C/C++** | ⭐⭐⭐☆☆ | Embedded, ESP32 |

### 🔒 Cybersecurity (CompTIA Stack)
- ✅ **CompTIA A+** (2024)
- ✅ **CompTIA Network+** (2024)  
- ✅ **CompTIA Linux+** (2024)
- ✅ **CompTIA Security+** (2024)
- ✅ **CompTIA Pentest+** (2024)

### 🛠️ Tools & Frameworks
- **Frontend:** React, TypeScript, HTML5/CSS3
- **Backend:** Node.js, Express, Spring Boot
- **Mobile:** Android (Kotlin), iOS (Swift)
- **IoT:** ESP32, Arduino, MQTT, Sensors
- **Security:** SIEM, SOC Operations, Penetration Testing
- **DevOps:** Git, Docker, CI/CD

---

## 📁 Unsere Projekte

### ✅ Fertige Projekte

| Projekt | Beschreibung | Tech Stack | Link |
|---------|-------------|------------|------|
| **🔷 Emin Portfolio** | Portfolio Website für Cybersecurity & Software Development | React, GitHub Pages | [Repo](https://github.com/emami-claw/emin-portfolio) |
| **🔒 SIEM Platform** | Security Information & Event Management mit Live-Dashboard | Node.js, React, MongoDB, WebSocket | [Repo](https://github.com/emami-claw/siem-platform) |
| **📋 Kanban Board Pro** | Professional Project Management mit Drag & Drop | React, @dnd-kit, Team Management | [Repo](https://github.com/emami-claw/kanban-board-pro) |
| **🔌 IoT Device Manager** | ESP32 Verwaltung mit OTA Updates & MQTT | Node.js, React, MQTT, WebSocket | [Repo](https://github.com/emami-claw/iot-device-manager) |

### ⏳ In Planung

| Projekt | Beschreibung | Status |
|---------|-------------|--------|
| **🚨 SOC Operations** | 24/7 Security Operations Center Dashboard | TODO |

---

## 🎯 Was wir machen

### 🔐 Cybersecurity
- **Penetration Testing** - Sicherheitsprüfungen & Schwachstellenanalyse
- **SIEM Implementation** - Log-Analyse & Threat Detection
- **SOC Operations** - 24/7 Security Monitoring
- **Security Audits** - Compliance & Sicherheitsbewertungen

### 💻 Software Development  
- **Full-Stack Web Apps** - React + Node.js + MongoDB
- **Mobile Apps** - Android (Kotlin) & iOS (Swift)
- **IoT Solutions** - ESP32, MQTT, Smart Home Integration
- **API Development** - REST & WebSocket APIs

### 🔌 Embedded Systems
- **ESP32 Firmware** - C++ Entwicklung für IoT Geräte
- **OTA Updates** - Over-The-Air Firmware Updates
- **Sensor Integration** - Temperatur, Feuchtigkeit, Gas-Sensoren
- **MQTT Kommunikation** - Real-time Datenübertragung

---

## 🏆 Ausbildung & Zertifikate

- 🎓 **Fachinformatiker Anwendungsentwicklung** (Java, Kotlin, Web)
- 🎓 **CompTIA Security Stack** (A+, Network+, Linux+, Security+, Pentest+)
- 📚 **Continuous Learning** - Immer up-to-date mit neuesten Technologien

---

## 📊 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=emami-claw&show_icons=true&theme=dark)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=emami-claw&layout=compact&theme=dark)

---

## 🤝 Zusammenarbeit

Wir arbeiten zusammen als gleichberechtigtes Team:
- **Abdi** - Cybersecurity Experte, Pentester
- **Emin** - Software Developer, IoT Spezialist

*Gemeinsam bauen wir sichere und innovative Lösungen!* 🚀

---

## 📫 Kontakt

- 📧 **Email:** info@emin-security.de
- 🌐 **Standort:** Berlin, Deutschland
- 💼 **GitHub:** [@emami-claw](https://github.com/emami-claw)

---

<div align="center">

**🔒 Security First | 💻 Code Second | 🚀 Always Learning**

*Built with ❤️ in Berlin*

</div>